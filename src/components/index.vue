<template>
	...
	<div class="content" ref="content" style="overflow:hidden">
		...
		<div class="center">
			...
			<img class="wheelBg" src="../assets/wheel1.png">
			<!-- <img ref="lightA" class="lightA" src="../assets/lighta.png" alt=""> -->
			<!-- <img ref="lightB" class="lightB" src="../assets/lighta.png" alt=""> -->
			<img ref="lightC" class="lightC" src="../assets/lightAA.png" alt="">
			<img ref="lightD" class="lightD" src="../assets/lightBB.png" alt="">
			<div class="wheel">
 		...
 		<p class="Ptitle" style="">
			<button type="button" name="button" data-toggle="modal" data-target=".bs-example-modal-lg" @click="changeStyle()"></button>
			<button type="button" name="button" data-toggle="modal" data-target=".bs-example-modal-lg" @click="selectMyWin()"></button>
		</p>
		<div class="ad" @click="jumpUrl()">
		</div>
<!-- 实物中奖区域 -->
		<div class="allWin" style="height: 7.8rem;top:40px" v-show='trueWin'>
				 ...
				 <div class="modal-body">
					 ...
					 <div v-if="myFood" class="myFood" style=" max-height:7rem;overflow: auto;">
						<p v-if="myWinInfo" v-for="item in myWinInfo">
				 			...
				 			<span class="spantwo" v-if="item.status=='1' && item.type=='1'">{{item.statusName}}</span>
				 			<span class="spantwo" v-if="item.status=='1' && item.type=='0'" @click="getXn()">{{item.statusName}}</span>
...
//				activeId:'',
		...
		methods: {
			...
			// 广告链接
			jumpUrl(){
				window.location.href="http://static.namifunds.com/qyl/web/views/invitation.html?iv_code=VGD959978"
			},
			change1(str){
				console.log(str);
			...
			selectMyWin(){
				...
				JSON.stringify({
					...
					if (res.body.receiveInfoDto.length!='0') {
						if(res.body.receiveInfoDto[0].type=="0" && res.body.receiveInfoDto[0].statusName=="已领取")
						{
							res.body.receiveInfoDto[0].statusName="查看"
						}
						...
						store.state.apdId=res.body.receiveInfoDto[0].apdIdmyWinInfo
